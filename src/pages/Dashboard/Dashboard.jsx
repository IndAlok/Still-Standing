...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      console.log(
        "Dashboard useEffect triggered, currentUser:",
        currentUser?.uid
      );
      ...
      try {
        ...
        try {
          const invitations = await crewConnectService.getUserInvitations(
            currentUser.uid
          );
        ...
        for (const group of userGroups.slice(0, 5)) {
          // Limit to 5 groups to avoid too many requests
          try {
            ...
            if (groupMessages && groupMessages.length > 0) {
              ...
              recentMessages.push({
                ...
                timestamp: latestMessage.sentAt?.toDate
                  ? latestMessage.sentAt.toDate()
            ...
            console.warn(
              `Failed to fetch messages for group ${group.id}:`,
              error
            );
          }
        }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        console.log(
          "Real-time invitation update: pending count =",
          pendingCount
        );
        setPendingInvitations(pendingCount);
      },
