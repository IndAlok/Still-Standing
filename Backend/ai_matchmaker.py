...
def load_csv_to_dataframe(filename, directory="."):
    ...
    Args:
        filename (str): Name of the CSV file (e.g., "participants.csv").
        directory (str): Directory path where the CSV file is located. Default is current directory.
...
class SmartMatchmakingEngineFAISS:
    ...
    # ---------- Helpers ----------
    def compute_experience_score(initiator_level, candidate_level):
       level_map = {"beginner": 1, "intermediate": 2, "advanced": 3}
       init_lvl = level_map.get(initiator_level, 2)  # default intermediate
    ...
    def preprocess_list_column(self, val):
        ...
        if isinstance(val, str):
            try:
                ...
                # Not a literal list; maybe comma separated
    ...
    def ensure_list(self, val):
        ...
        if isinstance(val, pd.Series):
            # If the Series is single-valued, extract it; else try to flatten unique values
            if len(val) == 1:
    ...
    def load_users_from_df(self, df: pd.DataFrame):
        ...
        # Map 'domain' -> 'domains' as list when needed
        ...
        if "experience_level" not in df.columns:
            df["experience_level"] = "medium"
        df["experience_level"] = df["experience_level"].fillna("medium").astype(str).str.lower()
        ...
        # Clustering (guard: need at least n_clusters samples)
        try:
    ...
    def find_matches(self, participant_id, num_matches=10, search_k=50):
        ...
        # Bound search_k to available users
        ...
        for j, idx in enumerate(candidate_indices):
            ...
            # Convert L2 distance → similarity (avoid division by zero)
    ...
    def form_instant_team(self, initiator_id, team_size=5, search_k=50):
        ...
        # Iteratively pick best candidate according to scoring function
        while len(team) < team_size:
            ...
            # Append candidate id and update coverage (ensure lists)
    ...
    def report_skill_gaps(self, team_ids):
        ...
        if not self.required_skills:
            # flatten lists of skills from all users
        ...
        coverage_percent = (
            round(len(covered_skills) / len(self.required_skills) * 100, 2)
...
if __name__ == "__main__":
   # participants_df, hacka = generate_simple_hackathon_dataset()
    participants_df = load_csv_to_dataframe("Backend/dataset/participants.csv", "")
    # Pick event requirements dynamically (instead of hardcoding)
    event = {"required_domains":["frontend","backend","ai_ml"],"required_skills":['react','Node.js', 'Python', 'Django','React',
             'JavaScript', 'HTML', 'CSS','AI/ML', 'Computer Vision', 'NLP']}
    #hackathons_df.iloc[1]  # Example: SuperHack 2025
